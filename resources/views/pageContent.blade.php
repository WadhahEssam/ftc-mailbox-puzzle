
<div style="padding: 37px;">
  <table class="mdl-data-table mdl-js-data-table mdl-data-table--selectable mdl-shadow--2dp">
    <thead>
      <tr>
        <th class="mdl-data-table__cell--non-numeric">Title</th>
        <th style="width: 100%" class="mdl-data-table__cell--non-numeric">Content</th>
        <th class="mdl-data-table__cell--non-numeric">Sent at</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td class="mdl-data-table__cell--non-numeric">Message #1</td>
        <td style="color:#940b0b" class="mdl-data-table__cell--non-numeric"><a>Re-enter The password to see the content</a></td>
        <td class="mdl-data-table__cell--non-numeric">Sat 9:45 PM</td>
      </tr>
      <tr>
        <td class="mdl-data-table__cell--non-numeric">Message #2</td>
        <td style="color:#940b0b" class="mdl-data-table__cell--non-numeric">Re-enter The password to see the content</td>
        <td class="mdl-data-table__cell--non-numeric">Fri 5:30 PM</td>
      </tr>
      <tr>
        <td class="mdl-data-table__cell--non-numeric">Message #3</td>
        <td style="color:#940b0b" class="mdl-data-table__cell--non-numeric">Re-enter The password to see the content</td>
        <td class="mdl-data-table__cell--non-numeric">Fri 7:16 AM</td>
      </tr>
    </tbody>
  </table>
</div>
